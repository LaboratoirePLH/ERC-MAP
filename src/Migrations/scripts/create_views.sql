-- Vues fr
CREATE
OR REPLACE VIEW vue_agent_activite AS
SELECT
...
CREATE
OR REPLACE VIEW vue_agent_agentivite AS
SELECT
    agent_agentivite.id_agent AS "Id Agent",
...
CREATE
OR REPLACE VIEW vue_agent_fr AS
SELECT
    ...
    string_agg(
        DISTINCT (agentivite.nom_fr) :: text,
        ', ' :: text
    ) AS "Agentivité",
...
CREATE
OR REPLACE VIEW vue_agent_genre AS
SELECT
...
CREATE
OR REPLACE VIEW vue_agent_nature AS
SELECT
...
CREATE
OR REPLACE VIEW vue_agent_statut AS
SELECT
...
CREATE
OR REPLACE VIEW vue_attestation_fr AS
SELECT
        ...
        (
            (
                (
                    ...
                    LEFT JOIN traduction_attestation ON (
                        (
                            traduction_attestation.id_attestation = attestation.id
                        )
                    )
                )
                LEFT JOIN formule ON ((formule.attestation_id = attestation.id))
            )
            LEFT JOIN attestation_pratique ON (
                (
                    attestation_pratique.id_attestation = attestation.id
                )
            )
        )
        ...
        (
            (
                (
                    (
                        contient_element
                        LEFT JOIN attestation ON (
                            (attestation.id = contient_element.id_attestation)
                        )
                    )
                    LEFT JOIN element ON ((element.id = contient_element.id_element))
                )
                LEFT JOIN genre_element ON (
                    (
                        genre_element.id = contient_element.id_genre_element
                    )
                )
            )
            LEFT JOIN nombre_element ON (
                (
                    nombre_element.id = contient_element.id_nombre_element
                )
            )
        )
        LEFT JOIN categorie_element ON (
            (
                categorie_element.id = contient_element.id_categorie_element
            )
        )
    )
ORDER BY
...
FROM
    (
        (
            ...
            LEFT JOIN categorie_element ON (
                (
                    categorie_element.id = element_categorie.id_categorie_element
                )
            )
        )
...
SELECT
    ...
    localisation.geom,
    string_agg(
        DISTINCT (q_fonction.nom_fr) :: text,
        ', ' :: text
    ) AS "Qualification fonctionnelle",
    string_agg(
                ...
                (
                    (
                        (
                            (
                                localisation
                                LEFT JOIN grande_region ON (
                                    (grande_region.id = localisation.grande_region_id)
                                )
                            )
                            LEFT JOIN sous_region ON ((sous_region.id = localisation.sous_region_id))
                        )
                        LEFT JOIN entite_politique ON (
                            (
                                entite_politique.id = localisation.entite_politique
                            )
                        )
                    )
...
CREATE
OR REPLACE VIEW vue_materiel_attestation_fr AS
SELECT
...
FROM
    (
        (
            ...
            LEFT JOIN categorie_materiel ON (
                (
                    categorie_materiel.id = attestation_materiel.id_categorie_materiel
                )
            )
        )
...
CREATE
OR REPLACE VIEW vue_occasion_attestation_fr AS
SELECT
...
FROM
    (
        (
            ...
            LEFT JOIN categorie_occasion ON (
                (
                    categorie_occasion.id = attestation_occasion.id_categorie_occasion
                )
            )
        )
...
CREATE
OR REPLACE VIEW vue_pratique_attestation_fr AS
SELECT
...
CREATE
OR REPLACE VIEW vue_source_fr AS
SELECT
    source.id,
                                                    ...
                                                    (
                                                        (
                                                            (
                                                                (
                                                                    source
                                                                    LEFT JOIN categorie_source ON (
                                                                        (categorie_source.id = source.categorie_source_id)
                                                                    )
                                                                )
                                                                LEFT JOIN categorie_materiau ON (
                                                                    (
                                                                        categorie_materiau.id = source.categorie_materiau_id
                                                                    )
                                                                )
                                                            )
                                                            LEFT JOIN materiau ON ((materiau.id = source.materiau_id))
                                                        )
                                                        LEFT JOIN categorie_support ON (
                                                            (
                                                                categorie_support.id = source.categorie_support_id
                                                            )
                                                        )
                                                    )
...
FROM
    (
        (
            ...
            LEFT JOIN type_source ON (
                (
                    source_type_source.id_type_source = type_source.id
                )
            )
        )
        LEFT JOIN categorie_source ON (
...
-- Vues en
CREATE
OR REPLACE VIEW vue_agent_en AS
SELECT
    agent.id AS "Id Agent",
    agent.id_attestation AS "Id Attestation",
    ...
    string_agg(
        DISTINCT (agentivite.nom_en) :: text,
        ', ' :: text
    ) AS "Agency",
...
CREATE
OR REPLACE VIEW vue_attestation_en AS
SELECT
...
CREATE
OR REPLACE VIEW vue_contient_element_en AS
SELECT
...
CREATE
OR REPLACE VIEW vue_element_en AS
SELECT
    row_number() OVER () AS "Id Element in testimony",
    contient_element.id_attestation AS "Id testimony",
    contient_element.id_element AS "Id element",
...
CREATE
OR REPLACE VIEW vue_source_en AS
SELECT
    source.id,
