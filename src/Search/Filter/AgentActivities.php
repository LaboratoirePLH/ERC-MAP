...
class AgentActivities extends AbstractFilter
{
    ...
    public static function filter(IndexRecherche $entity, array $criteria, array $sortedData): bool
    {
        ...
        // For each agent, we get its activities IDs
        $data = array_map(function ($el) {
            return array_column($el['activites'] ?? [], 'id');
        }, $agents);
        ...
        return !in_array(false, (array_map(function ($crit) use ($data) {
            ...
            $matched = 0;
            foreach ($data as $d) {
                // $d contains all the activities of an agent
