...
class AgentStatuses extends AbstractFilter {
    ...
    public static function filter(IndexRecherche $entity, array $criteria, array $sortedData): bool
    {
        ...
        // For each agent, we get its statuses IDs
        $data = array_map(function($el){
            return array_column($el['statutAffiches'] ?? [], 'id');
        }, $agents);
        ...
        return !!count(array_filter(array_map(function($crit) use ($data) {
            ...
            $matched = 0;
            foreach($data as $d){
                // $d contains all the statuses of an agent
