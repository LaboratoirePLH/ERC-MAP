...
class ElementInvariantCategories extends AbstractFilter
{
    ...
    public static function filter(IndexRecherche $entity, array $criteria, array $sortedData): bool
    {
        ...
        // We resolve the elements
        $elements = self::toArray(
            self::resolveElements($entity, $sortedData)
        );
        // For each element, we get its invariant categories IDs
        $data = array_map(function ($el) {
            return array_column($el['categories'] ?? [], 'id');
        }, $elements);
        ...
        return !in_array(false, (array_map(function ($crit) use ($data) {
            ...
            $matched = 0;
            foreach ($data as $d) {
                // $d contains all the invariant categories of an element
