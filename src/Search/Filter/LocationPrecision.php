...
class LocationPrecision extends AbstractFilter {
    ...
    {
        if(!is_null($location['nomSite'] ?? null)){ return 1; }
        if(!is_null($location['nomVille'] ?? null)){ return 2; }
        if(!is_null($location['sousRegion'] ?? null)){ return 3; }
        if(!is_null($location['grandeRegion'] ?? null)){ return 4; }
        return 0;
    }
}